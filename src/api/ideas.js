...
export default ({config, db, broadcast }) => {
	...
	api.get('/notes/:from_label/:rel_label/:optional_to_label/:optional_entity_key/:optional_entity_val/:return_entity', (req, res) => {
		...
		console.log(query);
		let getSubgraph = FullfillIdeaView(SubGraph, query, return_entity)
		getSubgraph.then(()=> res.send(SubGraph))
		getSubgraph.catch((err)=> res.status(500).send(err))
	})

	function FullfillIdeaView(SubGraph, query, return_entity) {
		return new Promise(function (resolve, reject) {
			db.cypher({
				query: query
			}, function (err, results) {
				if (err || !results || results.length == 0 || results[0] == null) {
					reject({ message: "No Idea notes" })
				} else {
					results.map((result) => {
					...
					let Expand = ExpandIdeasCard(SubGraph, SubGraph.node_label_index.Note);
	...
	function ExpandIdeasCard(SubGraph, profile_uuid_array) {
		return new Promise(function (resolve, reject) {
