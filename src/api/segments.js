...
export default ({ config, db, broadcast }) => {
	...
	function GetSegmentSubGraph(SubGraph, uuid) {
		return new Promise(function (resolve, reject) {
			let GetPivot = GraphAPI.AddRemoteNodeToSubgraph(uuid, SubGraph, db);
			GetPivot.then(() => {
				let Outgoing = GraphAPI.GetOutgoingRels([uuid], SubGraph, 'segment_audiences', db);
	...
	//Returns a Segment Centered SubGraph, filtered to only Direct Workshop Components
	api.get('/connections/:segment_uuid', (req, res) => {
		var SubGraph = {};
		GetSegmentSubGraph(SubGraph, req.params.segment_uuid).then(() => { res.send(SubGraph) })
	});
	...
	api.post('/connections', (req, res) => {
		...
		["Extra", "Missing"].forEach((state) => {
			SegmentWorkshopComponents.forEach((it) => {
					...
					Difference[state].node_label_index[it.label].forEach((node_id) => {
						req.body[(state == "Extra") ? "revised" : "initial"].incoming[it.type][node_id].forEach((rel_id) => {
							...
							let query = (state == "Extra") ?
								`MATCH (j:Segment {uuid:'${rel.from_id}'}), (o {uuid:'${rel.to_id}'}) CREATE (j)-[e:${rel.type} {uuid:'${rel.properties.uuid}', order:${rel.properties.order || 0}, created_at:${new Date().getTime()}}]->(o)`
								:
