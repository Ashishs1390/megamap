(function (factory) {
  ...
  function ChartJsFactory (ChartJs, $timeout) {
    ...
    function scatterChart (scope) {
      var setScatterChartDefaults = function(data) {
        data.borderColor     = data.borderColor     || 'transparent';
        data.backgroundColor = data.backgroundColor || 'transparent';
        return;
      }
      if(Array.isArray(scope.chartData[0])) {
        scope.chartDatasetOverride = scope.chartDatasetOverride || [];
        scope.chartData.forEach(function(dataset, idx) {
          scope.chartDatasetOverride[idx] ?
            setScatterChartDefaults(scope.chartDatasetOverride[idx]) :
            scope.chartDatasetOverride.push(setScatterChartDefaults({}));
        });
      } else {
        scope.chartDatasetOverride = scope.chartDatasetOverride || {};
        setScatterChartDefaults(scope.chartDatasetOverride);
      }
      return;
    }

    function getChartData (type, scope) {
      if(type === 'scatter') scatterChart(scope);
